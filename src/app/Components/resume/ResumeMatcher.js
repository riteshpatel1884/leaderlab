...
function getUsageData() {
  try {
    return JSON.parse(localStorage.getItem(STORAGE_KEY)) || { count: 0, date: getISTDateStr() };
...
// ── Score ring — uses currentColor trick so it works on any bg ───────────────
function ScoreRing({ score, size = 100 }) {
      ...
      {weights.map(({ key, label, points }) => {
        ...
        return (
          <div key={key} style={{
            ...
            border: `1px solid ${present ? "var(--border)" : "var(--red-dim)"}`,
            borderRadius: 7,
          }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{
                  ...
                  background: present ? "var(--bg-card)" : "var(--red-dim)",
                  color: present ? "var(--text-muted)" : "var(--red)",
                  border: present ? "1px solid var(--border)" : "1px solid rgba(var(--red), 0.2)",
                }}>
                  {present ? `${earned}/${points}` : `0/${points}`}
                </span>
...
function Tag({ text, variant }) {
  const styles = {
    miss: { bg: "var(--red-dim)",   color: "var(--red)",   border: "var(--border)" },
    hit:  { bg: "var(--green-dim)", color: "var(--green)", border: "var(--border)" },
  };
  const s = styles[variant];
  return (
    <span style={{
  ...
  return (
    <>
      ...
      <div style={{
        ...
        borderRadius: 12, width: "90%", maxWidth: 660, maxHeight: "85vh",
        overflowY: "auto", zIndex: 1000, boxShadow: "0 20px 60px rgba(0,0,0,0.3)",
      }} onClick={e => e.stopPropagation()}>
        <div style={{
        ...
        {history.length === 0 ? (
          ...
          <div style={{ padding: "14px 18px" }}>
            {history.map((entry) => {
              const expYears = entry.experience === "" || entry.experience === null ? null : parseInt(entry.experience);
              return (
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 4, flexWrap: "wrap" }}>
                        ...
                        {expYears !== null && (
                          <span style={{
                            ...
                            background: "var(--bg-card)", padding: "2px 7px", borderRadius: 3,
                        ...
                        {entry.jobType && (
                          <span style={{
                            fontSize: 11,
                            color: entry.jobType === "internship" ? "var(--blue)" : "var(--green)",
                            background: entry.jobType === "internship" ? "var(--blue-dim)" : "var(--green-dim)",
...
export default function ResumeMatcher() {
  ...
  const analyze = async () => {
    ...
    const sectionScoresTemplate = allSectionKeys.map(key =>
      `    "${key}": { "present": <bool>, "qualityScore": <0-100 or null if not scored>, "quality": "strong|weak|missing", "note": "<brief specific note about this section vs JD>" }`
    ).join(",\n");
    ...
    try {
      const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1600,
          messages: [
            { role: "system", content: "You are a blunt expert technical recruiter. Return ONLY valid JSON — no markdown, no preamble." },
            { role: "user", content: prompt },
          ],
        }),
      });
  ...
  return (
    <div>
      <style>{`
        @keyframes spin { to { transform: rotate(360deg); } }
        * { box-sizing: border-box; }
        .rm-input:focus { border-color: var(--accent) !important; background: var(--bg-card) !important; }
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{
            ...
            {remaining}/{DAILY_LIMIT}
            <div style={{ fontSize: 9, color: "var(--text-muted)", fontWeight: 600, letterSpacing: "0.5px" }}>
              left today
            </div>
          </div>
          <button
      ...
      <div style={{
        background: "var(--bg-hover)",
        border: `1px solid ${isInternship ? "var(--blue-dim)" : "var(--border)"}`,
        borderRadius: 8, padding: "12px 16px", marginBottom: 20,
      }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div style={{
            fontSize: 10, fontWeight: 700, textTransform: "uppercase",
            letterSpacing: "1px", color: isInternship ? "var(--blue)" : "var(--text-muted)",
          }}>
            Scoring breakdown — {isInternship ? "Internship" : "Full-Time Job"} (100 pts total)
          </div>
          {isInternship && (
            <span style={{
              fontSize: 10, color: "var(--blue)", background: "var(--blue-dim)",
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {currentWeights.map(({ label, points }) => (
            <span key={label} style={{
              ...
              {label}{" "}
              <span style={{ fontWeight: 700, color: isInternship ? "var(--blue)" : "var(--text-primary)" }}>
                {points}pt
              </span>
            </span>
          ))}
        ...
        {isInternship && (
          <p style={{ margin: "10px 0 0", fontSize: 10, color: "var(--blue)", lineHeight: 1.5, opacity: 0.75 }}>
            ℹ️ Internship scoring emphasises Projects (30), Skills (20) and Certifications (20) — work experience carries no weight since freshers are not expected to have any.
          </p>
        )}
      </div>
      ...
      {isLimitReached && (
        <div style={{
          ...
          fontSize: 12, color: "var(--red)", opacity: 0.85,
        }}>
          Daily limit reached — resets at midnight IST.
        </div>
      )}
      {remaining === 1 && !isLimitReached && (
        <div style={{
          ...
          borderRadius: 7, padding: "10px 14px", marginBottom: 14,
          fontSize: 12, color: "var(--yellow)", opacity: 0.85,
        }}>
          Last analysis for today — use it wisely.
        </div>
      )}
          ...
          <div>
            ...
            <div style={{
              ...
              {[{ value: "job", label: "Full-Time Job" }, { value: "internship", label: "Internship" }]
                .filter(({ value }) => value === "job" || isFresherOrUnset)
                .map(({ value, label }) => (
                  <button key={value} onClick={() => setJobType(value)} disabled={isLimitReached}
                    style={{
                      padding: "9px 12px", borderRadius: 4, cursor: "pointer",
                      border: jobType === value
                        ? `1px solid ${value === "internship" ? "var(--blue)" : "var(--accent-border)"}`
                        : "1px solid transparent",
                      background: jobType === value
                        ? value === "internship" ? "var(--blue-dim)" : "var(--accent-dim)"
                        : "transparent",
                      color: jobType === value
                        ? value === "internship" ? "var(--blue)" : "var(--accent)"
                        : "var(--text-muted)",
                      fontSize: 12, fontWeight: jobType === value ? 700 : 500,
                      transition: "all 0.2s", opacity: isLimitReached ? 0.4 : 1,
                    }}
                  >
                    {label}
                  </button>
      ...
      {error && (
        <div style={{
          ...
          color: "var(--red)", marginBottom: 14, opacity: 0.85,
        }}>
          {error}
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 32, alignItems: "center" }}>
        <button
          ...
          style={{
            ...
            background: loading || isLimitReached ? "var(--bg-hover)" : "var(--accent)",
            color: loading || isLimitReached ? "var(--text-muted)" : "#050f0c",
            fontSize: 13, fontWeight: 700,
            cursor: isLimitReached ? "not-allowed" : "pointer",
            opacity: loading || isLimitReached ? 0.6 : 1,
            transition: "all 0.2s",
          }}
        >
          {loading ? (
        ...
        <div>
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <p style={{ margin: 0, fontSize: 14, fontWeight: 600, color: "var(--text-primary)", lineHeight: 1.5 }}>
                {result.verdict}
              </p>
              <div style={{ display: "flex", gap: 7, marginTop: 8, flexWrap: "wrap" }}>
                ...
                <span style={{
                  ...
                  background: result._jobType === "internship" ? "var(--blue-dim)" : "var(--green-dim)",
                  color: result._jobType === "internship" ? "var(--blue)" : "var(--green)",
                  border: "1px solid var(--border)",
                }}>
                  {result._jobType === "internship" ? "Internship" : "Job"}
                </span>
                {expYears !== null && (
                  <span style={{
                    fontSize: 11, padding: "2px 8px", borderRadius: 3,
                    background: expYears === 0 ? "var(--red-dim)" : "var(--green-dim)",
                    color: expYears === 0 ? "var(--red)" : "var(--green)",
                    border: "1px solid var(--border)",
                  }}>
                    {expYears === 0 ? "Fresher" : `${expYears}yr exp`}
          ...
          {result.atsRisk && (
            <div style={{
              ...
              borderRadius: "0 0 8px 8px", marginBottom: 24,
            }}>
              <span style={{
                fontSize: 10, fontWeight: 700, textTransform: "uppercase",
                letterSpacing: "0.8px", color: "var(--text-muted)", marginRight: 10,
              }}>
                ATS Risk
              </span>
          ...
          {result.missingSections?.length > 0 && (
            <div style={{
              ...
              background: "var(--red-dim)", border: "1px solid var(--border)", borderRadius: 7,
            }}>
              <div style={{
                fontSize: 10, fontWeight: 700, textTransform: "uppercase",
                letterSpacing: "1px", color: "var(--red)", marginBottom: 8,
              }}>
                Scored sections not found in resume
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {result.missingSections.map((s, i) => (
                  <span key={i} style={{
                    fontSize: 12, padding: "4px 10px", borderRadius: 4, fontWeight: 600,
                    background: "var(--bg-card)", border: "1px solid var(--border)", color: "var(--red)",
                  }}>
                    ✕ {s}
                  </span>
                ))}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            <div>
              <div style={{
                fontSize: 10, fontWeight: 700, textTransform: "uppercase",
                letterSpacing: "0.8px", color: "var(--text-muted)", marginBottom: 8,
              }}>Missing</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          ...
          {result.oneThingToFix && (
            <>
              ...
              <div style={{
                ...
                <div style={{
                  fontSize: 10, fontWeight: 700, textTransform: "uppercase",
                  letterSpacing: "1px", color: "var(--accent)", marginBottom: 7,
                }}>
                  Do this before applying
                </div>
