...
function QuickStats({ applications }) {
  const stats = useMemo(() => {
    ...
    return { total, interviews, offers, rejected, rate, bestPlatform, bestRate };
  }, [applications]);
...
export default function ApplicationsTable({
  ...
  const filtered = useMemo(() => {
    return applications
      .filter((app) => {
        ...
        const matchJob =
          jobTypeFilter === "All" || app.jobType === jobTypeFilter;
        return matchSearch && matchStatus && matchPlatform && matchWork && matchJob;
      })
      .sort((a, b) => {
        if (sortBy === "date")
          return new Date(b.createdAt) - new Date(a.createdAt);
  ...
  const getDaysAgo = (dateStr) => {
    if (!dateStr) return null;
    const days = Math.round((now - new Date(dateStr)) / (1000 * 60 * 60 * 24));
    if (days === 0) return "Today";
    if (days === 1) return "1d ago";
    return `${days}d ago`;
  };
                    ...
                    return (
                      ...
                      >
                        ...
                        <td>
                          ...
                          {/* Show rejection reason inline in table */}
