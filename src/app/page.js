...
//   { id: 27, company: "Productboard",role: "PM",                    status: "Offer",  platform: "Company site", workType: "Remote",  resumeVersion: "v1", dateApplied: "2025-03-16" },
...
//               <strong>{stats.resumeData[0]?.name}</strong> has a {stats.resumeData[0]?.rate}% callback rate — it's your strongest resume right now.
//             </div>
//           </div>
...
//   const [loading, setLoading] = useState(false);
//   const [stats, setStats] = useState(null);
//   // Bumped on every "See it in action" run so charts that depend on
//   // ResponsiveContainer (recharts) get a brand-new DOM subtree instead
//   // of reusing/recycling the previous one. Reusing the old subtree is
//   // what caused the "Work type split" donut legend to visually overlap
//   // with stale content on some desktop widths/resizes.
...
//           <div className="ll-chart-card">
//             <div className="ll-chart-title">Application funnel</div>
//             <div className="ll-funnel-flow">
//               {[
//                 { label: "Applied",   count: stats.total,               color: "#60a5fa" },
//                 { label: "Interview", count: stats.byStatus.Interview,  color: "#f59e0b" },
//                 { label: "Offer",     count: stats.byStatus.Offer,      color: "#34d399" },
//               ].map((step, i, arr) => {
//                 const prev = i === 0 ? stats.total : arr[i - 1].count;
//                 const conv = i > 0 && prev > 0 ? ((step.count / prev) * 100).toFixed(0) : null;
//                 const share = stats.total > 0 ? (step.count / stats.total) * 100 : 0;
//                 return (
//                   <div className="ll-funnel-item" key={step.label}>
//                     {i > 0 && (
//                       <div className="ll-funnel-connector">
//                         {/* Horizontal arrow — desktop/tablet layout (row) */}
//                         <svg className="ll-connector-arrow ll-connector-h" width="28" height="24" viewBox="0 0 28 24" aria-hidden="true">
//                           <path d="M2 12 H24 M18 6 L24 12 L18 18" fill="none" stroke="var(--border-light)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
//                         </svg>
//                         {/* Vertical arrow — mobile layout, funnel stacks top-to-bottom */}
//                         <svg className="ll-connector-arrow ll-connector-v" width="24" height="28" viewBox="0 0 24 28" aria-hidden="true">
//                           <path d="M12 2 V24 M6 18 L12 24 L18 18" fill="none" stroke="var(--border-light)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
//                         </svg>
//                         {conv && <span className="ll-funnel-conv-badge">{conv}%</span>}
//                       </div>
//                     )}
//                     <div className="ll-funnel-block" style={{ flexGrow: Math.max(step.count, 0.6), background: step.color }}>
//                       <div className="ll-funnel-block-value">{step.count}</div>
//                       <div className="ll-funnel-block-label">{step.label}</div>
//                       <div className="ll-funnel-block-share">{share.toFixed(0)}% of total</div>
//                     </div>
//                   </div>
//                 );
//               })}
//             </div>
//             <div className="ll-funnel-rejected-chip">
//               <span className="ll-funnel-rejected-dot" />
//               <span>
//                 Rejected — {stats.byStatus.Rejected} application{stats.byStatus.Rejected === 1 ? "" : "s"} (
//                 {stats.total > 0 ? ((stats.byStatus.Rejected / stats.total) * 100).toFixed(0) : 0}%)
//               </span>
//             </div>
//           </div>

//           {/* ── Donuts with pre-shown legends (value + % of total, on every screen size) ── */}
//           <div className="ll-donut-grid">
//             <div className="ll-chart-card">
//               <div className="ll-chart-title">Status distribution</div>
//               <div className="ll-donut-row">
//                 <div className="ll-pie-wrap ll-pie-wrap-sm" key={`status-pie-${analysisId}`}>
//                   <ResponsiveContainer width="100%" height="100%">
//                     <PieChart tabIndex={-1}>
//                       <Pie
//                         isAnimationActive={false}
...
//                   </ResponsiveContainer>
//                 </div>
//                 <div className="ll-donut-legend">
//                   {Object.entries(stats.byStatus).filter(([, v]) => v > 0).map(([name, value]) => (
//                     <div className="ll-legend-row" key={name}>
//                       <div className="ll-legend-left">
//                         <span className="ll-legend-dot" style={{ background: STATUS_COLORS[name] }} />
//                         <span className="ll-legend-name">{name}</span>
//                       </div>
//                       <div className="ll-legend-right">
//                         <span className="ll-legend-pct">
//                           {stats.total > 0 ? ((value / stats.total) * 100).toFixed(0) : 0}%
//                         </span>
//                         <span className="ll-legend-value" style={{ color: STATUS_COLORS[name] }}>{value}</span>
//                       </div>
//                     </div>
//                   ))}
//                 </div>
...
//               <div className="ll-chart-title">Work type split</div>
//               <div className="ll-donut-row">
//                 <div className="ll-pie-wrap ll-pie-wrap-sm" key={`worktype-pie-${analysisId}`}>
//                   <ResponsiveContainer width="100%" height="100%">
//                     <PieChart tabIndex={-1}>
//                       <Pie
//                         isAnimationActive={false}
...
//                 </div>
//                 <div className="ll-donut-legend">
//                   {stats.workTypeData.map((e) => (
//                     <div className="ll-legend-row" key={e.name}>
//                       <div className="ll-legend-left">
//                         <span className="ll-legend-dot" style={{ background: e.fill }} />
//                         <span className="ll-legend-name">{e.name}</span>
//                       </div>
//                       <div className="ll-legend-right">
//                         <span className="ll-legend-pct">
//                           {stats.total > 0 ? ((e.value / stats.total) * 100).toFixed(0) : 0}%
//                         </span>
//                         <span className="ll-legend-value" style={{ color: e.fill }}>{e.value}</span>
//                       </div>
//                     </div>
//                   ))}
//                 </div>
...
//         /* Chart cards — isolation + overflow:hidden prevents any child
//            (recharts SVG, absolutely positioned legend, tooltip portal,
//            etc.) from ever visually bleeding into a sibling card, even
//            during a resize or a remount. */
//         .ll-chart-card { background: var(--bg-card); border: 1px solid var(--border); border-radius: 10px; padding: 20px; margin-bottom: 16px; position: relative; isolation: isolate; overflow: hidden; }
//         .ll-chart-title { font-family: 'Manrope', sans-serif; font-size: 11px; font-weight: 700; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.8px; margin-bottom: 16px; }
//         .ll-legend-label { font-size: 11px; color: var(--text-secondary); font-family: 'Inter', sans-serif; }

//         /* ── Legend-with-values (donut charts) ──
//            NOTE: this row previously only reliably showed the donut
//            side-by-side with the legend on wide desktop screens, but
//            at intermediate widths (roughly 640–1024px — small laptops,
//            tablets, half-split browser windows) the legend column got
//            squeezed hard enough that the value/percentage text was
//            effectively clipped or crammed unreadably. We now force the
//            donut + legend to stack (full width legend) well before that
//            squeeze point, and every legend row shows both the % of total
//            and the raw count on every screen size. */
//         .ll-donut-row { display: flex; align-items: center; gap: 18px; min-width: 0; }
//         .ll-pie-wrap-sm { width: 140px; height: 140px; flex-shrink: 0; position: relative; overflow: hidden; }
//         .ll-donut-legend { display: flex; flex-direction: column; gap: 8px; flex: 1 1 220px; min-width: 220px; }
//         .ll-legend-row {
...
//           min-width: 0;
//         }
//         .ll-legend-row:hover { border-color: var(--border); background: var(--bg-hover); }
//         .ll-legend-left { display: flex; align-items: center; gap: 8px; min-width: 0; flex: 1 1 auto; overflow: hidden; }
//         .ll-legend-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
//         .ll-legend-name { font-size: 12.5px; color: var(--text-secondary); font-family: 'Inter', sans-serif; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
//         .ll-legend-right { display: flex; align-items: center; gap: 10px; flex-shrink: 0; }
//         .ll-legend-pct { font-family: 'IBM Plex Mono', monospace; font-size: 11px; font-weight: 500; color: var(--text-muted); min-width: 32px; text-align: right; }
//         .ll-legend-value { font-family: 'IBM Plex Mono', monospace; font-weight: 700; font-size: 13px; flex-shrink: 0; white-space: nowrap; }

//         /* Donut + legend stack (full-width legend) starting at 1024px,
//            so numbers never get squeezed on tablets/small laptops —
//            only a true wide desktop keeps them side by side. */
//         @media (max-width: 1024px) {
//           .ll-donut-row   { flex-direction: column; align-items: stretch; gap: 14px; }
//           .ll-pie-wrap-sm { width: 130px; height: 130px; margin: 0 auto; }
//           .ll-donut-legend{ min-width: 0; }
//         }

//         /* ── Interactive funnel ── */
//         .ll-funnel-flow { display: flex; align-items: stretch; gap: 0; }
//         .ll-funnel-item { display: flex; align-items: center; flex: 0 0 auto; }
//         .ll-funnel-connector { display: flex; flex-direction: column; align-items: center; gap: 4px; padding: 0 6px; flex-shrink: 0; }
//         .ll-funnel-conv-badge { font-family: 'IBM Plex Mono', monospace; font-size: 10px; font-weight: 700; color: var(--text-muted); white-space: nowrap; }
//         .ll-funnel-block {
//           border-radius: 10px; padding: 16px 14px; min-width: 90px; flex: 1;
//           display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 2px;
//           cursor: default; transition: transform 0.18s ease, box-shadow 0.18s ease;
//         }
//         .ll-funnel-block:hover { transform: translateY(-3px); box-shadow: 0 8px 20px rgba(0,0,0,0.18); }
//         .ll-funnel-block-value { font-family: 'IBM Plex Mono', monospace; font-size: 22px; font-weight: 700; color: #04120d; line-height: 1; }
//         .ll-funnel-block-label { font-size: 11.5px; font-weight: 700; color: rgba(4,18,13,0.75); margin-top: 4px; }
//         .ll-funnel-block-share { font-size: 9.5px; color: rgba(4,18,13,0.55); font-family: 'IBM Plex Mono', monospace; margin-top: 1px; }
//         .ll-funnel-rejected-chip { display: flex; align-items: center; gap: 8px; margin-top: 16px; font-size: 12px; color: var(--text-muted); font-family: 'IBM Plex Mono', monospace; }
//         .ll-funnel-rejected-dot { width: 8px; height: 8px; border-radius: 50%; background: var(--red); flex-shrink: 0; }

//         /* Connector arrows — swapped via CSS based on viewport (see the
//            mobile media query below). Horizontal shows on desktop/tablet
//            (row layout), vertical shows on mobile (column layout). */
//         .ll-connector-arrow.ll-connector-h { display: block; }
//         .ll-connector-arrow.ll-connector-v { display: none; }

//         .ll-donut-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 16px; align-items: start; }
//         .ll-pie-wrap { width: 100%; height: 200px; min-width: 0; overflow: hidden; }
...
//           .ll-pie-wrap-sm       { width: 120px; height: 120px; margin: 0 auto; }

//           /* Funnel stacks vertically on mobile, so the connector between
//              steps must point down instead of right (Applied → Interview,
//              then Interview ↓ Offer). */
//           .ll-funnel-flow       { flex-direction: column; align-items: stretch; }
//           .ll-funnel-item       { flex-direction: column; width: 100%; }
//           .ll-funnel-connector  { flex-direction: row; padding: 4px 0; justify-content: center; }
//           .ll-connector-arrow.ll-connector-h { display: none; }
//           .ll-connector-arrow.ll-connector-v { display: block; }
//           .ll-funnel-block      { width: 100%; padding: 12px 8px; }
//           .ll-funnel-block-value{ font-size: 18px; }
...
/* ─── Donut Chart (hero preview + dashboard showcase) ─── */
function DonutChart({ size = 100 }) {
  const data = [
    { pct: 0.48, color: "#60a5fa" }, { pct: 0.16, color: "#f59e0b" },
    { pct: 0.08, color: "#34d399" }, { pct: 0.28, color: "#f87171" },
...
/* ─────────────────────────────────────────────────────────
   Shared dashboard mockup markup — used at small size in the
   hero, and at a larger size in the DashboardShowcase section
...
/* ─────────────────────────────────────────────────────────
   Dashboard showcase section — replaces the old "Live demo"
   section. That section re-ran the exact same funnel / donut /
   trend charts the hero already shows, just interactively, so
   it read as a duplicate rather than new information. This
   section instead gives the hero's dashboard mockup room to
   breathe as the page's second beat, at a larger size.
───────────────────────────────────────────────────────── */
  ...
  return (
    <>
      <style>{`
        ...
        /* ── Hero ──
           Single centered column: the hero used to sit next to a
           right-side dashboard mockup, but that's been removed, so
           the copy now centers itself in the full hero width instead
           of hugging the left half. */
        .ll-hero {
          position: relative;
        ...
        .ll-hero-copy { padding-top: 6px; display: flex; flex-direction: column; align-items: center; }
        ...
        .ll-hero-desc { font-size: 16px; color: var(--text-secondary); line-height: 1.7; max-width: 480px; margin: 0 auto 30px; }
        .ll-hero-ctas { display: flex; gap: 12px; flex-wrap: wrap; justify-content: center; margin-bottom: 22px; }
        .ll-hero-trust {
        ...
        /* ── Mock Dashboard preview (shared by hero + showcase section) ── */
        .mock-dashboard-wrap { position: relative; }
        .mock-dashboard {
          background: var(--bg-card); border: 1px solid var(--border); border-radius: 14px;
          overflow: hidden;
        ...
        .mock-chrome-dots span { width: 8px; height: 8px; border-radius: 50%; display: inline-block; }
        .mock-chrome-dots span:nth-child(1) { background: #ff5f57; }
        .mock-chrome-dots span:nth-child(2) { background: #ffbd2e; }
        ...
        /* ── Dashboard showcase section (replaces the old interactive
        ...
        .ll-showcase-dashboard .mock-stat-label { font-size: 10px; }
        .ll-showcase-dashboard .mock-stat-value { font-size: 25px; }
        .ll-showcase-dashboard .mock-charts { gap: 14px; }
        .ll-showcase-dashboard .mock-chart-card { padding: 18px 20px; }
        .ll-showcase-dashboard .mock-chart-title { font-size: 10px; margin-bottom: 14px; }
        .ll-showcase-dashboard .funnel-box { font-size: 17px; padding: 14px 8px; }
        .ll-showcase-dashboard .funnel-label { font-size: 9px; }
        .ll-showcase-dashboard .funnel-rejected { font-size: 9px; }
        .ll-showcase-dashboard .legend-item { font-size: 12px; }
        .ll-showcase-dashboard .legend-val { font-size: 12px; }
